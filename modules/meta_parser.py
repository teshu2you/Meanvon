...
class MetadataParser(ABC):
    ...
    def set_data(self, raw_prompt, full_prompt, raw_negative_prompt, full_negative_prompt, steps, base_model_name,
        ...
        print(f"loras: {loras}")
        for iii in loras:
            lora_enable, lora_name, lora_weight = iii[0], iii[1], iii[2]
            if lora_name != 'None':
...
class MeanVonMetadataParser(MetadataParser):
    def get_scheme(self) -> MetadataScheme:
        return MetadataScheme.MEANVON
...
def get_exif(metadata: str | None, metadata_scheme: str):
    ...
    # 0x0131 = Software
    exif[0x0131] = 'MeanVon v' + version.main_version
    # 0x927C = MakerNote
