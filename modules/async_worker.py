...
def worker():
    ...
    from modules.util import remove_empty_str, HWC3, resize_image, \
        get_image_shape_ceil, set_image_shape_ceil, get_shape_ceil, resample_image, erode_or_dilate, ordinal_suffix, get_enabled_loras
    ...
    def get_image(path, megapixels=1.0):
        # there is sonmething with the syntax. TODOCheck
        image = None
    ...
    def build_image_wall(async_task):
        ...
        for img in results:
            ...
            if not isinstance(img, np.ndarray):
                return
            # the image 's dimension is 3, if not, just return
            if img.ndim != 3:
                return
            results.append(img)

        # shape is the image numpy array's size
    ...
    def handler(async_task):
        ...
        for inx, val in enumerate(args):
            ...
            print(f"{inx}:{name}:{val}")

        save_metadata_json = args.pop()
        save_metadata_image = args.pop()
        img2img_mode = args.pop()
        img2img_start_step = args.pop()
        img2img_denoise = args.pop()
        img2img_scale = args.pop()
        revision_mode = args.pop()
        positive_prompt_strength = args.pop()
        negative_prompt_strength = args.pop()
        revision_strength_1 = args.pop()
        revision_strength_2 = args.pop()
        revision_strength_3 = args.pop()
        revision_strength_4 = args.pop()
        control_lora_canny = args.pop()
        canny_edge_low = args.pop()
        canny_edge_high = args.pop()
        canny_start = args.pop()
        canny_stop = args.pop()
        canny_strength = args.pop()
        canny_model = args.pop()
        control_lora_depth = args.pop()
        depth_start = args.pop()
        depth_stop = args.pop()
        depth_strength = args.pop()
        depth_model = args.pop()
        input_gallery = args.pop()
        revision_gallery = args.pop()
        keep_input_names = args.pop()
        base_clip_skip = args.pop()
        refiner_clip_skip = args.pop()
        outpaint_expansion_ratio = args.pop()
        ...
        image_factory_checkbox = args.pop()
        ...
        save_metadata_to_images = args.pop() if not adapter.args_manager.args.disable_metadata else False
        metadata_scheme = MetadataScheme(
            args.pop()) if not adapter.args_manager.args.disable_metadata else MetadataScheme.FOOOCUS
        ...
        for _ in range(flags.controlnet_image_count):
            ...
            if cn_img is not None:
                print(cn_type.__str__())
                print(cn_img.__str__())
                print(cn_stop.__str__())
                print(cn_weight.__str__())
                cn_tasks[cn_type].append([cn_img, cn_stop, cn_weight])
        print(cn_tasks.__str__())
        ...
        if performance_selection == Performance.LCM:
            print('Enter LCM mode.')
            progressbar(async_task, 1, 'Downloading LCM components ...')
            loras += [[modules.config.downloading_sdxl_lcm_lora(), 1.0]]

            if refiner_model_name != 'None':
                print(f'Refiner disabled in LCM mode. <{refiner_model_name}>')

            if refiner_model_name != 'Not Exist!->':
                print(f'Refiner disabled in LCM mode. <{refiner_model_name}>')
        ...
        if performance_selection == 'TURBO':
            ...
            progressbar(async_task, 1, 'Downloading TURBO components ...')
            loras += [[modules.config.downloading_sdxl_turbo_lora(), 1.0]]

            if refiner_model_name != 'None':
                print(f'Refiner disabled in TURBO mode. <{refiner_model_name}>')
        ...
        if performance_selection == Performance.LIGHTNING:
            print('Enter Lightning mode.')
            progressbar(async_task, 1, 'Downloading Lightning components ...')
            print('force to replace the 1st lora.')
            loras[0] = [modules.config.downloading_sdxl_lightning_lora(), 1.0]
        ...
        if image_factory_checkbox:
                ...
                if isinstance(inpaint_image, np.ndarray) and isinstance(inpaint_mask, np.ndarray) \
                    ...
                    if inpaint_parameterized:
                        ...
                        inpaint_head_model_path, inpaint_patch_model_path = modules.config.downloading_inpaint_models(
                            inpaint_engine)
                        base_model_additional_loras += [(True, inpaint_patch_model_path, 1.0)]
            ...
            if current_tab == 'ip' or \
                    mixing_image_prompt_and_inpaint or \
        ...
        if not skip_prompt_processing:
            ...
            if revision_mode:
                ...
                revision_strengths = [revision_strength_1, revision_strength_2, revision_strength_3,
                                      revision_strength_4]
                for i in range(revision_gallery_size):
                    progressbar(async_task, 4, f'Revision for image {i + 1} ...')
        ...
        if 'vary' in goals:
            ...
            #  denoising_strength is tuned for purpose
        ...
        if 'inpaint' in goals:
            if len(outpaint_selections) > 0:
                ...
                if 'top' in outpaint_selections:
                    inpaint_image = np.pad(inpaint_image, [[int(H * outpaint_expansion_ratio), 0], [0, 0], [0, 0]],
                                           mode='edge')
                    inpaint_mask = np.pad(inpaint_mask, [[int(H * outpaint_expansion_ratio), 0], [0, 0]],
                                          mode='constant',
                                          constant_values=255)
                if 'bottom' in outpaint_selections:
                    inpaint_image = np.pad(inpaint_image, [[0, int(H * outpaint_expansion_ratio)], [0, 0], [0, 0]],
                                           mode='edge')
                    inpaint_mask = np.pad(inpaint_mask, [[0, int(H * outpaint_expansion_ratio)], [0, 0]],
                                          mode='constant',
                                          constant_values=255)

                H, W, C = inpaint_image.shape
                if 'left' in outpaint_selections:
                    inpaint_image = np.pad(inpaint_image, [[0, 0], [int(W * outpaint_expansion_ratio), 0], [0, 0]],
                                           mode='edge')
                    inpaint_mask = np.pad(inpaint_mask, [[0, 0], [int(H * outpaint_expansion_ratio), 0]],
                                          mode='constant',
                                          constant_values=255)
                if 'right' in outpaint_selections:
                    inpaint_image = np.pad(inpaint_image, [[0, 0], [0, int(W * outpaint_expansion_ratio)], [0, 0]],
                                           mode='edge')
                    inpaint_mask = np.pad(inpaint_mask, [[0, 0], [0, int(H * outpaint_expansion_ratio)]],
                                          mode='constant',
                                          constant_values=255)
        ...
        async_task.yields.append(['preview', (13, 'Moving model to GPU ...', None, 0, image_number)])

        def callback(step, x0, x, total_steps, y):
            done_steps = current_task_id * steps + step
            img_p = current_task_id + 1
            img_r = image_number - img_p
        ...
        for current_task_id, task in enumerate(tasks):
            ...
            try:
                ...
                if performance_selection == constants.TYPE_LIGHTNING and 1 == 2:
                    imgs = pipeline.lightning_process_diffusion(
                ...
                metadata = {
                    ...
                    'steps': steps, 'switch': switch, 'sharpness': sharpness, 'cfg': cfg_scale,
                ...
                if control_lora_canny:
                    metadata |= {
                        'canny_edge_low': canny_edge_low, 'canny_edge_high': canny_edge_high,
                        'canny_start': canny_start,
                        'canny_stop': canny_stop, 'canny_strength': canny_strength, 'canny_model': canny_model,
                        'canny_input': input_image_filename
                    }
                if control_lora_depth:
                    metadata |= {
                        'depth_start': depth_start, 'depth_stop': depth_stop, 'depth_strength': depth_strength,
                        'depth_model': depth_model, 'depth_input': input_image_filename
                    }
                metadata |= {'software': version.main_version}
                ...
                for x in imgs:
                    ...
                    d += [
                        ('Image-2-Image',
                         (img2img_mode, start_step, denoise, img2img_scale, input_image_filename) if img2img_mode else (
                             img2img_mode)),
                        ('Revision', (revision_mode, revision_strength_1, revision_strength_2, revision_strength_3,
                                      revision_strength_4, revision_images_filenames) if revision_mode else (
                            revision_mode)),
                        ...
                        ('Canny', (control_lora_canny, canny_edge_low, canny_edge_high, canny_start, canny_stop,
                                   canny_strength, canny_model, input_image_filename) if control_lora_canny else (
                            control_lora_canny)),
                        ('Depth', (control_lora_depth, depth_start, depth_stop, depth_strength, depth_model,
                                   input_image_filename) if control_lora_depth else (control_lora_depth)),
                        ('Seed', task['task_seed'])
                    ]
                    ...
                    d.append(('Version', 'v' + version.main_version))
                    d.append(('Execution Time', f'{execution_time:.2f} seconds'))

                    base_model_name_prefix = base_model_name.split(".")[0]
                    img_paths.append(log(x, d, 3, metadata_string, save_metadata_json, save_metadata_image, keep_input_names,
                        input_image_filename, output_format, base_model_name_prefix))
        ...
        async_task.yields.append(['metadatas', metadata_strings])
        async_task.yields.append(['results', results])
