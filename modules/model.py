...
from modules.img_tool import enlarge_image, shrink_image, crop_to_nearest_multiple_of_n, \
    image_pipeline_func
from modules.settings import creat_video_by_opencv
from modules.settings import img_resize_to_HW, auto_adjust_img
from modules.config import path_videos_outputs as vid_output_folder
...
def infer(
    ...
    device, model = modules.model_setting.load_video_module(model_load_flag)
    if model is None:
        print(f"{vid_output_folder}")
        return vid_output_folder
