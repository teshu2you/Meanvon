...
class ConditioningAverage:
    ...
    def addWeighted(self, conditioning_to, conditioning_from, conditioning_to_strength):
        ...
        if len(conditioning_from) > 1:
            printF(name=MasterName.get_master_name(), info="Warning: ConditioningAverage conditioning_from contains more than 1 cond, only the first one will actually be applied to conditioning_to.").printf()
...
class ConditioningConcat:
    ...
    def concat(self, conditioning_to, conditioning_from):
        ...
        if len(conditioning_from) > 1:
            printF(name=MasterName.get_master_name(), info="Warning: ConditioningConcat conditioning_from contains more than 1 cond, only the first one will actually be applied to conditioning_to.").printf()
...
class SaveLatent:
    ...
    def save(self, samples, filename_prefix="ldm_patched", prompt=None, extra_pnginfo=None):
        ...
        metadata = None
        if not args_parser.args.disable_metadata:
...
def load_custom_node(module_path, ignore=set()):
    ...
    if os.path.isfile(module_path):
        ...
        module_name = sp[0]
    try:
        printF(name=MasterName.get_master_name(), info="Trying to load custom node {}".format(module_path)).printf()
        if os.path.isfile(module_path):
        ...
        if hasattr(module, "NODE_CLASS_MAPPINGS") and getattr(module, "NODE_CLASS_MAPPINGS") is not None:
            ...
            printF(name=MasterName.get_master_name(), info="Skip {} module for custom nodes due to the lack of NODE_CLASS_MAPPINGS.".format(module_path)).printf()
            return False
    except Exception as e:
        printF(name=MasterName.get_master_name(), info="{}".format(traceback.format_exc())).printf()
        printF(name=MasterName.get_master_name(), info="Cannot import {} module for custom nodes: {}".format(module_path, e.__str__())).printf()
        return False
...
def init_external_custom_nodes():
    ...
    if len(node_import_times) > 0:
        printF(name=MasterName.get_master_name(), info="\nImport times for custom nodes:").printf()
        for n in sorted(node_import_times):
            if n[2]:
                import_message = ""
            else:
                import_message = " (IMPORT FAILED)"
            printF(name=MasterName.get_master_name(), info="{:6.1f} seconds{}: {}".format(n[0], import_message, n[1])).printf()
...
def init_builtin_extra_nodes():
    ...
    if len(import_failed) > 0:
        ...
        for node in import_failed:
            printF(name=MasterName.get_master_name(), info="IMPORT FAILED: {}".format(node)).printf()
        printF(name=MasterName.get_master_name(), info="\nThis issue might be caused by new missing dependencies added the last time you updated ComfyUI.").printf()
        if args_parser.args.windows_standalone_build:
            printF(name=MasterName.get_master_name(), info="Please run the update script: update/update_comfyui.bat").printf()
        else:
            printF(name=MasterName.get_master_name(), info="Please do a: pip install -r requirements.txt").printf()
