            ...
            if first_op_done == False:
                ...
                if cleared_cache == False:
                    ...
                    printF(name=MasterName.get_master_name(),
                           info="out of memory error, emptying cache and trying again").printf()
                    continue
                steps *= 2
                if steps > 64:
                    raise e
                printF(name=MasterName.get_master_name(),
                       info="out of memory error, increasing steps and trying again {}".format(steps)).printf()
            else:
                raise e
...
if model_management.xformers_enabled():
    ...
    if args.use_split_cross_attention:
        ...
        printF(name=MasterName.get_master_name(),
               info="Using sub quadratic optimization for cross attention, if you have memory or speed issues try using: --use-split-cross-attention").printf()
