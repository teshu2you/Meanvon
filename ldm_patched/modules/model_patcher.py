...
class ModelPatcher:
    def __init__(self, model, load_device, offload_device, size=0, weight_inplace_update=False):
        ...
        if not hasattr(self.model, 'device'):
            printF(name=MasterName.get_master_name(),
                   info="Model doesn't have a device attribute.").printf()
        ...
        self.model.current_device = self.model.device
        self.model.current_loaded_device = self.model.device
        self.patches = {}
        self.backup = {}
        self.object_patches = {}
    ...
    def current_device(self):
        return self.model.current_device
    ...
    def clone_has_same_weights(self, clone):
        ...
        if self.patches_uuid == clone.patches_uuid:
            if len(self.patches) != len(clone.patches):
                printF(name=MasterName.get_master_name(),
                       info="WARNING: something went wrong, same patch uuid but different length of patches.").printf()
            else:
                return True
    ...
    def load(self, device_to=None, lowvram_model_memory=0, force_patch_weights=False, full_load=False):
        ...
        if lowvram_counter > 0:
            printF(name=MasterName.get_master_name(),
                   info="loaded partially!  {} - {} - {}".format(lowvram_model_memory / (1024 * 1024), mem_counter / (1024 * 1024), patch_counter)).printf()
            self.model.model_lowvram = True
        else:
            printF(name=MasterName.get_master_name(),
                   info="loaded completely!  {} - {} - {}".format(lowvram_model_memory / (1024 * 1024), mem_counter / (1024 * 1024), full_load)).printf()
            self.model.model_lowvram = False
            if full_load:
        ...
        if self.model.device != device_to:
    ...
    def calculate_weight(self, patches, weight, key, intermediate_dtype=torch.float32):
        printF(name=MasterName.get_master_name(),
               info="WARNING the ModelPatcher.calculate_weight function is deprecated, please use: comfy.lora.calculate_weight instead").printf()
