...
def load_lora(lora, to_load):
    ...
    for x in lora.keys():
        if x not in loaded_keys:
            printF(name=MasterName.get_master_name(), info="lora key not loaded: {}".format(x)).printf()
...
def model_lora_keys_unet(model, key_map={}):
    ...
    if isinstance(model, KModel):
        model = model.config
        model_unet_config = model.unet_config
    else:
        model_unet_config = model.model_config.unet_config
    if isinstance(model, ldm_patched.modules.model_base.SD3) or isinstance(model, backend.huggingface_guess.model_list.SD3): #Diffusers lora SD3
        diffusers_keys = ldm_patched.modules.utils.mmdit_to_diffusers(model_unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
    ...
    if isinstance(model, ldm_patched.modules.model_base.AuraFlow) or isinstance(model, backend.huggingface_guess.model_list.AuraFlow): #Diffusers lora AuraFlow
        diffusers_keys = ldm_patched.modules.utils.auraflow_to_diffusers(model_unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
    ...
    if isinstance(model, ldm_patched.modules.model_base.HunyuanDiT) or isinstance(model, backend.huggingface_guess.model_list.HunyuanDiT) or isinstance(model, backend.huggingface_guess.model_list.HunyuanDiT1):
        for k in sdk:
            if k.startswith("diffusion_model.") and k.endswith(".weight"):
                key_lora = k[len("diffusion_model."):-len(".weight")]
                key_map["base_model.model.{}".format(key_lora)] = k #official hunyuan lora format

    if isinstance(model, ldm_patched.modules.model_base.Flux) or isinstance(model, backend.huggingface_guess.model_list.Flux): #Diffusers lora Flux
        diffusers_keys = ldm_patched.modules.utils.flux_to_diffusers(model_unet_config, output_prefix="diffusion_model.")
        for k in diffusers_keys:
            if k.endswith(".weight"):
                to = diffusers_keys[k]
    ...
    for p in patches:
        ...
        if patch_type == "diff":
            ...
            if do_pad_weight and diff.shape != weight.shape:
                printF(name=MasterName.get_master_name(), info="Pad weight {} from {} to shape: {}".format(key, weight.shape, diff.shape)).printf()
                weight = pad_tensor_to_shape(weight, diff.shape)

            if strength != 0.0:
                if diff.shape != weight.shape:
                    printF(name=MasterName.get_master_name(), info="WARNING SHAPE MISMATCH {} WEIGHT NOT MERGED {} != {}".format(key, diff.shape, weight.shape)).printf()
                else:
            ...
            try:
                ...
                printF(name=MasterName.get_master_name(), info="ERROR {} {} {}".format(patch_type, key, e)).printf()
