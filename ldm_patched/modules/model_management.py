...
if args.always_cpu:
    ...
    printF(name=MasterName.get_master_name(), info="Running on {} CPU threads".format(torch.get_num_threads())).printf()
...
printF(name=MasterName.get_master_name(),
       info="[Info] Total VRAM {:0.0f} MB, total RAM {:0.0f} MB".format(total_vram, total_ram)).printf()
if not args.always_normal_vram and not args.always_cpu:
    if lowvram_available and total_vram <= 4096:
        printF(name=MasterName.get_master_name(),
               info="[Info] Trying to enable lowvram mode because your GPU seems to have 4GB or less. If you don't want this use: --always-normal-vram").printf()
...
if args.disable_xformers:
    ...
    try:
        ...
        try:
            ...
            printF(name=MasterName.get_master_name(),
                   info="[Info] xformers version: {}".format(XFORMERS_VERSION)).printf()
...
if args.all_in_fp32:
    printF(name=MasterName.get_master_name(),
           info="[Info] Forcing FP32, if this improves things please report it.").printf()
    FORCE_FP32 = True

if args.all_in_fp16:
    printF(name=MasterName.get_master_name(),
           info="[Info] Forcing FP16.").printf()
    FORCE_FP16 = True
...
printF(name=MasterName.get_master_name(), info="[Info] Set vram state to: {}".format(vram_state.name)).printf()

ALWAYS_VRAM_OFFLOAD = args.always_offload_from_vram

if ALWAYS_VRAM_OFFLOAD:
    printF(name=MasterName.get_master_name(), info="[Info] Always offload VRAM").printf()
...
try:
    printF(name=MasterName.get_master_name(),
           info="[Info] Device: {}".format(get_torch_device_name(get_torch_device()))).printf()
except:
    printF(name=MasterName.get_master_name(),
           info="[Info] Could not pick default device.").printf()

printF(name=MasterName.get_master_name(),
       info="[Info] VAE dtype: {}".format(VAE_DTYPE)).printf()
...
class LoadedModel:
    ...
    def model_load(self, lowvram_model_memory=0):
        ...
        if lowvram_model_memory > 0:
            printF(name=MasterName.get_master_name(),
                   info="[Info] loading in lowvram mode: {} MB".format(lowvram_model_memory / (1024 * 1024))).printf()
            mem_counter = 0
            for m in self.real_model.modules():
                if hasattr(m, "ldm_patched_cast_weights"):
                    ...
                    printF(name=MasterName.get_master_name(),
                           info="[Info] lowvram: loaded module regularly: {}".format(m)).printf()

            self.model_accelerated = True

        if is_intel_xpu() and not args.disable_ipex_hijack:
            self.real_model = torch.xpu.optimize(self.real_model.eval(), inplace=True, auto_kernel_selection=True,
                                                 graph_mode=True)
...
def unet_manual_cast(weight_dtype, inference_device):
    ...
    fp16_supported = ldm_patched.modules.model_management.should_use_fp16(inference_device,
