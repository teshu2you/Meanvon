...
class ControlLora(ControlNet):
    ...
    def pre_run(self, model, percent_to_timestep_function):
        ...
        self.control_model = ldm_patched.cldm.cldm.ControlNet(**controlnet_config)
...
def load_controlnet_mmdit(sd):
    ...
    control_model = ldm_patched.cldm.mmdit.ControlNet(num_blocks=num_blocks, operations=operations, device=offload_device, dtype=unet_dtype, **model_config.unet_config)
...
def load_t2i_adapter(t2i_data):
    ...
    if "body.0.in_conv.weight" in keys:
        ...
        if cin == 256 or cin == 768:
            xl = True
        model_ad = ldm_patched.t2i_adapter.adapter.Adapter(cin=cin, channels=[channel, channel*2, channel*4, channel*4][:4], nums_rb=2, ksize=ksize, sk=True, use_conv=use_conv, xl=xl)
