...
# [["", 1],["", 1],["", 1]]
#  [{"model_name": "sd_xl_offset_example-lora_1.0.safetensors", "weight": 0.5}]

# LoraList = TypeAdapter(List[Lora])
default_loras_model = [Lora(lora_enable=lora[0], model_name=lora[1], weight=lora[2]) for lora in default_loras if
                       lora[1] != 'None']
# print(default_loras_model)
# default_loras_json = LoraList.dump_json(default_loras_model)

default_loras_json = [{"lora_enable": True, "model_name": lora[1], "weight": lora[2]} for lora in default_loras if
                      lora[1] != 'None']
...
class AdvancedParams(BaseModel):
    ...
    overwrite_vary_strength: float = Field(-1, description='Forced Overwrite of Denoising Strength of "Vary"', ge=-1,
                                           le=1.0)
    overwrite_upscale_strength: float = Field(-1, description='Forced Overwrite of Denoising Strength of "Upscale"',
                                              ge=-1, le=1.0)
...
class Text2ImgRequest(BaseModel):
    ...
    webhook_url: str | None = Field(default='',
                                    description="Optional URL for a webhook callback. If provided, the system will send a POST request to this URL upon task completion or failure."
...
class ImgUpscaleOrVaryRequest(Text2ImgRequest):
    ...
    def as_form(cls, input_image: UploadFile = Form(description="Init image for upsacale or outpaint"),
                ...
                upscale_value: float | None = Form(None, description="Upscale custom value, None for default value",
                                                   ge=1.0, le=5.0),
                prompt: str = Form(''),
                ...
                refiner_switch: float = Form(default=default_refiner_switch, description="Refiner Switch At", ge=0.1,
                                             le=1.0),
...
class ImgInpaintOrOutpaintRequest(Text2ImgRequest):
    ...
    def as_form(cls, input_image: UploadFile = Form(description="Init image for inpaint or outpaint"),
        ...
        return cls(input_image=input_image, input_mask=input_mask, inpaint_additional_prompt=inpaint_additional_prompt,
                   ...
                   outpaint_distance_bottom=outpaint_distance_bottom, prompt=prompt, negative_prompt=negative_prompt,
...
class ImgPromptRequest(ImgInpaintOrOutpaintRequest):
    ...
    def as_form(cls, input_image: UploadFile = Form(File(None), description="Init image for inpaint or outpaint"),
        ...
        return cls(input_image=input_image, input_mask=input_mask, inpaint_additional_prompt=inpaint_additional_prompt,
                   ...
                   outpaint_distance_left=outpaint_distance_left, outpaint_distance_right=outpaint_distance_right,
                   outpaint_distance_top=outpaint_distance_top, outpaint_distance_bottom=outpaint_distance_bottom,
                   image_prompts=image_prompts, prompt=prompt, negative_prompt=negative_prompt,
                   style_selections=style_selection_arr,
                   performance_selection=performance_selection, aspect_ratios_selection=aspect_ratios_selection,
                   image_number=image_number, image_seed=image_seed, sharpness=sharpness, guidance_scale=guidance_scale,
                   base_model_name=base_model_name, refiner_model_name=refiner_model_name,
...
class QueryJobRequest(BaseModel):
    ...
    require_step_preview: bool = Field(False,
...
class AsyncJobResponse(BaseModel):
    ...
    job_step_preview: str | None = Field(None,
