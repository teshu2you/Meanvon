...
def render():
    ...
    with gr.Row():
        TEXT_INPUT = gr.Textbox(
            label=get('TextInputLabel'), lines=10, placeholder=get('TextInputPlaceholder'), value=get('default_text'),interactive=True
        )
        TXT_FILE_INPUT = gr.Files(
            label=get('TxtFileInputLabel'), type="file", file_types=['.txt','.srt'], visible=BATCH_PROCESSING_CHECKBOX.value,interactive=True
        )
