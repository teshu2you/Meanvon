    ...
    for file, content in text_segments.items():
        ...
        for i, segment in enumerate(content):
            ...
            if not isinstance(segment, str):
                segment = str(segment)
            segment_audio_path, sample_rate, audio_data = audio_processor(CHAT, file_name, segment, refine_text_flag,
                                                                          nums2text_switch, params_refine_text,
                                                                          params_infer_code, i)
...
def audio_processor(chat, file_name, segment, refine_text_flag, nums2text_switch, params_refine_text, params_infer_code,
                    i):
    if refine_text_flag:
