...
def metrics_decoration(func):
    def wrap_func(progress=gr.Progress(track_tqdm=True), *args, **kwargs):
        ...
        print(f">>>[matt 🧠]: Generation finished in {timestamp.split(',')[0]} ({(stop_time - start_time)} seconds)")
        return result
...
def exif_writer_png(exif_datas, filename):
    datas = PngInfo()
    datas.add_text("UserComment", f"matt settings: {exif_datas}")
...
def lora_model_list(model):
    if (("XL" in model.upper()) or ("SSD-1B" in model.upper()) or ("SEGMIND-VEGA" in model.upper()) or (
