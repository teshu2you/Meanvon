...
from backend.patcher_diffusion.kolors.models.modeling_chatglm import ChatGLMModel
...
class KolorsTextProcessingEngine:
    def __init__(
        ...
        model_embeddings = text_encoder.transformer.embedding
        model_embeddings.token_embedding = model_embeddings.word_embeddings
    ...
    def encode_with_transformers(self, tokens):
        ...
        z = outputs.hidden_states[-2].permute(1, 0, 2).clone()
        t = outputs.hidden_states[-1][-1, :, :].clone()     # [batch_size, 4096]
        ...
        bs_embed = t.shape[0]
        pooled_output = t.repeat(1, 1).view(bs_embed, -1)

        # pooled_output = torch.cat((torch.zeros(size=[pooled_output.shape[0], 1536], device=pooled_output.device), pooled_output), dim=1)
    ...
    def process_tokens(self, remade_batch_tokens, batch_multipliers):
        ...
        # tokens = tokens.view(tokens.shape[0], tokens.shape[1])
