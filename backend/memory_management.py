...
if args.pytorch_deterministic:
    printF(name=MasterName.get_master_name(),
           info="Using deterministic algorithms for pytorch").printf()
...
if args.directml is not None:
    ...
    printF(name=MasterName.get_master_name(),
           info="Using directml with device: {}".format(torch_directml.device_name(device_index))).printf()
...
printF(name=MasterName.get_master_name(), info="Total VRAM {:0.0f} MB, total RAM {:0.0f} MB".format(total_vram, total_ram)).printf()

try:
    printF(name=MasterName.get_master_name(),
           info="pytorch version: {}".format(torch.version.__version__)).printf()
except:
    pass
...
if args.disable_xformers:
    ...
    try:
        ...
        try:
            ...
            printF(name=MasterName.get_master_name(),
                   info="xformers version: {}".format(XFORMERS_VERSION)).printf()
            if XFORMERS_VERSION.startswith("0.0.18"):
                printF(name=MasterName.get_master_name(),
                       info="\nWARNING: This version of xformers has a major bug where you will get black images when generating high resolution images.").printf()
                printF(name=MasterName.get_master_name(),
                       info="Please downgrade or upgrade xformers to a different version.\n").printf()
...
if args.all_in_fp32:
    printF(name=MasterName.get_master_name(),
           info="Forcing FP32, if this improves things please report it.").printf()
    FORCE_FP32 = True

if args.all_in_fp16:
    printF(name=MasterName.get_master_name(),
           info="Forcing FP16.").printf()
    FORCE_FP16 = True
...
printF(name=MasterName.get_master_name(), info="Set vram state to: {vram_state.name}").printf()

ALWAYS_VRAM_OFFLOAD = args.always_offload_from_vram

if ALWAYS_VRAM_OFFLOAD:
    printF(name=MasterName.get_master_name(),
           info="Always offload VRAM").printf()

PIN_SHARED_MEMORY = args.pin_shared_memory

if PIN_SHARED_MEMORY:
    printF(name=MasterName.get_master_name(),
           info="Always pin shared GPU memory").printf()
...
try:
    ...
    printF(name=MasterName.get_master_name(),
           info="Device: {}".format(torch_device_name)).printf()
except:
    torch_device_name = ''
    printF(name=MasterName.get_master_name(),
           info="Could not pick default device.").printf()

if 'rtx' in torch_device_name.lower():
    if not args.cuda_malloc:
        printF(name=MasterName.get_master_name(),
               info="Hint: your device supports --cuda-malloc for potential speed improvements.").printf()
...
class LoadedModel:
    ...
    def model_load(self, model_gpu_memory_when_using_cpu_swap=-1):
        ...
        if do_not_need_cpu_swap:
            ...
            printF(name=MasterName.get_master_name(),
                   info="{} Swap Loaded ({} method): {:.2f} MB, GPU Loaded: {:.2f} MB".format(swap_flag, method_flag,swap_counter / (1024 * 1024), mem_counter / (1024 * 1024))).printf()
...
def free_memory(memory_required, device, keep_loaded=[], free_all=False):
    if free_all:
        ...
        printF(name=MasterName.get_master_name(),
               info="[Unload] Trying to free all memory for {} with {} models keep loaded ... ".format(device, len(keep_loaded))).printf()
    else:
        printF(name=MasterName.get_master_name(),
               info="[Unload] Trying to free {:.2f} MB for {} with {} models keep loaded ... ".format(memory_required / (1024 * 1024), device, len(keep_loaded))).printf()
    ...
    for i in range(len(current_loaded_models) - 1, -1, -1):
        if not offload_everything:
            ...
            printF(name=MasterName.get_master_name(),
                   info="Current free memory is {:.2f} MB ... ".format(free_memory / (1024 * 1024))).printf()
        ...
        if shift_model.device == device:
            if shift_model not in keep_loaded:
                ...
                printF(name=MasterName.get_master_name(),
                       info="Unload model {} ".format(m.model.model.__class__.__name__)).printf()
                m.model_unload()
                del m
                unloaded_model = True
...
def load_models_gpu(models, memory_required=0, hard_memory_preservation=0, minimum_memory_required=None):
    global vram_state
    ...
    if len(models_to_load) == 0:
        ...
        if moving_time > 0.1:
            printF(name=MasterName.get_master_name(),
                   info="Memory cleanup has taken {:.2f} seconds".format(moving_time)).printf()
    ...
    for loaded_model in models_to_load:
        ...
        if lowvram_available and (vram_set_state == VRAMState.LOW_VRAM or vram_set_state == VRAMState.NORMAL_VRAM):
            ...
            printF(name=MasterName.get_master_name(),
                   info="[Memory Management] Target: {}, Free GPU: {:.2f} MB, Model Require: {:.2f} MB, Previously Loaded: {:.2f} MB, Inference Require: {:.2f} MB, Remaining: {:.2f} MB, ".format(loaded_model.model.model.__class__.__name__,current_free_mem / (1024 * 1024),model_require / (1024 * 1024),previously_loaded / (1024 * 1024),memory_for_inference / (1024 * 1024),estimated_remaining_memory / (1024 * 1024))).printf()
    ...
    moving_time = time.perf_counter() - execution_start_time
    printF(name=MasterName.get_master_name(),
           info="Moving model(s) has taken {:.2f} seconds".format(moving_time)).printf()
...
printF(name=MasterName.get_master_name(), info="VAE dtype preferences: {} -> {}".format(VAE_DTYPES, vae_dtype())).printf()
