...
class W8A16Linear(torch.autograd.Function):
    ...
    def forward(ctx, inp: torch.Tensor, quant_w: torch.Tensor, scale_w: torch.Tensor, weight_bit_width):
        if inp.device == torch.device("cpu"):
            inp = inp.to(torch.cuda.current_device()).to(dtype=torch.float16)
        if quant_w.device == torch.device("cpu"):
            quant_w = quant_w.to(torch.cuda.current_device())
        if scale_w.device == torch.device("cpu"):
            scale_w = scale_w.to(torch.cuda.current_device())
...
class QuantizedLinear(torch.nn.Module):
    ...
    def forward(self, input):
        ...
        if self.bias is not None:
            output = output.to(device=self.bias.device)
